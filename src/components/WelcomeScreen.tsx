...
import { Zap, FolderOpen, ArrowRight, ExternalLink, Archive } from 'lucide-react';
import { clsx } from 'clsx';
import { open as openDialog } from '@tauri-apps/plugin-dialog';
import { useWorkspaceStore } from '../store/workspaceStore';

/**
 * First-run / re-pick UI. Shown when no workspace is configured (or the
 * stored path no longer exists on disk).
 *
 * Flow:
 *   1. Suggest <Documents>/Autoflow.
 *   2. User can Browse to override.
 *   3. If there's legacy appData content from a pre-workspace install, offer
 *      to move it into the new workspace.
 *   4. Click "Continue" → workspace.json is written, the rest of the app
 *      bootstraps normally.
 */
    ...
    try {
      ...
      if (legacy && doMigrate) {
        try { await migrateLegacy(); }
          ...
          // Non-fatal — workspace is still configured. Just report.
          console.warn('[welcome] migration failed:', e);
        }
      }
      onDone();
    } catch (e) {
      ...
      >
        ...
        <div className="px-7 pt-7 pb-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <div className="text-[10px] font-mono tracking-[0.18em] uppercase text-accent-soft">
                Welcome
              </div>
              <div className="text-[18px] font-bold font-display text-ink leading-tight">
                Pick your work directory
              </div>
            </div>
          </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <label className="inline-flex items-center gap-2 mt-2 text-[11.5px] text-ink-dim cursor-pointer">
                  <input
                    type="checkbox"
                    checked={doMigrate}
                    onChange={e => setDoMigrate(e.target.checked)}
        ...
        <div className="flex items-center justify-between px-7 py-5">
          <p className="text-[10.5px] font-mono text-ink-ghost flex items-center gap-1">
            ...
            machine state (window size, etc.) stays in appData
          </p>
          <button
            onClick={confirm}
